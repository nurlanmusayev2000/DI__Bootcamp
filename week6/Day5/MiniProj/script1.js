...
function createElement() {
  ...
  let elements = fromLocalStorage?.map((val) => {
    ...
    if (dateconfEnd < new Date()) {
      element = ` <p onclick="displayDescription()" class = "name ${
        val.checking
      }">Name:${val.name}</p>
                <p class="taskDescription active">Description: ${
                  val.description
                } </p>    
<p>Starting date: ${dateconfStart} </p>
<p>${
        dateconfEnd.getDate() - dateconfStart.getDate()
      } days letf to complete the task</p>`;
    } else {
      element = ` <p onclick="displayDescription() class = "name ${
        val.checking
      }" >Name:${val.name}</p>
            ...
            <p>Starting date: ${dateconfStart} </p>
            <p>${
              dateconfEnd.getDate() - new Date().getDate()
            } days letf to complete the task</p>`;
    }
...
// !!! function isComplete() NOT working
// !!! function displayDescription() not working
