...
class ImagePart extends Component{
	...
	async	componentDidMount(){
			...
			const fethingData = await	fetch(`https://api.unsplash.com/search/photos?page=1&query=$mountain&client_id=cMfOWDJ9dpbDwILKLjXER9g8V6Ngp2STUD5lDDpjzOI`)
	...
	async componentDidUpdate(prevState , prevProps){
		...
		if (this.props.imgType!==prevState.imgType) {
			console.log("fetxh ucun",this.props);
			const fethingData = await	fetch(`https://api.unsplash.com/search/photos?page=1&query=${this.props.imgType}&client_id=cMfOWDJ9dpbDwILKLjXER9g8V6Ngp2STUD5lDDpjzOI`)
			const result = await fethingData.json();
			console.log("netice:",result);
			this.props.getAllImages(result)
		}
